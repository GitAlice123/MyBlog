    ...
    <el-main>
      <el-form ref="articleForm" :model="article" label-width="120px" @submit.prevent="submitArticle">
        ...
        <el-form-item label="上传标题图片">
          <el-upload class="avatar-uploader" action="http://localhost:8080/api/upload-image" :show-file-list="false"
            :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
            <template #trigger>
              <div class="avatar-uploader-trigger">
                <!-- 使用 v-show 来判断是否显示图片或默认图标 -->
                <img v-show="article.title_img_path" :src="article.title_img_path" class="avatar">
                <i v-show="!article.title_img_path" class="el-icon-plus avatar-uploader-icon"></i>
              </div>
            </template>
          </el-upload>
  ...
  methods: {
    ...
    handleAvatarSuccess(response) {
      ...
      if (response.code === 1) {
        ...
        console.log('标题图片路径:', this.article.title_img_path);
      } else {
